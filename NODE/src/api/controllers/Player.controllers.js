...
const toggleTeam = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { teams } = req.body;
    const playerById = await Player.findById(id);
    if (playerById) {
      const arrayIdTeams = teams.split(",");
      Promise.all(
        arrayIdTeams.map(async (team, index) => {
          if (playerById.teams.includes(team)) {
            try {
        ...
        .then(async () => {
          return res.status(200).json({
            dataUpdate: await Player.findById(id).populate("teams"),
          });
        });
    } else {
...
const getAll = async (req, res, next) => {
  try {
    const allPlayer = await Player.find().populate("teams");
...
module.exports = { createPlayer, toggleTeam, deletePlayer, getById, getAll, getByName };